...
\medskip
We formalize the notions of graph patterns for both directed edge-labelled graphs and property graphs. In such definitions, $\textbf{Var}$ refers to a countably infinite set of \textit{variables} ranging over the set of constants in $\textbf{Con}$. We refer generically to constants and variables as \textit{terms}, denoted as $\textbf{Term} = \textbf{Con} \cup \textbf{Var}$.

\subsubsection{Graph patterns}
At the basis of every structured query language of graphs are \textit{(basic) graph patterns}, that are related with the graph model that is being queried. In directed edge-labelled graphs the terms are its nodes and edge labels. The terms in property graphs are its ids, labels, properties and values.
...
\begin{description}
    \item[Homomorphism-based semantics] 
    allows multiple variables to be mapped to the same term.
    \item[Isomorphic-based semantics] requires nodes and edge variables to be mapped to distinct terms. 
\end{description}
